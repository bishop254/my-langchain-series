...
const prompt = ChatPromptTemplate.fromMessages([
  [
    "system",
    "You are a helpful assistant. You have access to tools to add numbers and search the web and perform a customer lookup. Only use tools if explicitly asked or if they are necessary to answer the question.",
  ],
  ["human", "{query}"],
]);

const processUserQueryWithTools = async (query: string = "") => {
  console.log("Processing query:\n", query);

  try {
    const aiMsg = await prompt
      .invoke({ query })
      .then((formattedPrompt) => modelWithTools.invoke(formattedPrompt));

    console.log("AI first pass:\n", aiMsg.content);
    ...
    if (aiMsg.tool_calls && aiMsg.tool_calls.length > 0) {
      ...
      console.log(
        `Tool to call: ${toolCall.name}, Args: ${JSON.stringify(toolCall.args)}`,
      );

      let toolResult: unknown;

      if (toolCall.name === add_numbers_tool.name) {
        toolResult = await add_numbers_tool.invoke(
          toolCall.args as { a: number; b: number },
        );
      } else if (toolCall.name === tavily_search_tool.name) {
        ...
        const searchResults = await tavily_search_tool.invoke(searchQuery);

        const contentToSummarize = JSON.parse(searchResults)
          .map((r: any) => r.content)
          .join("\n\n");
        ...
        const summaryPrompt = ChatPromptTemplate.fromMessages([
          [
            "system",
            `You are a summarization assistant. Summarize the following content concisely based on the original user query. Content: ${contentToSummarize}`,
          ],
          ["human", `Original query: ${query}`],
        ]);

        const summaryReponse = await summaryPrompt
          .pipe(gptModel)
          .pipe(new StringOutputParser())
          .invoke({});
        toolResult = `Summary of web search: ${summaryReponse}`;
      } else if (toolCall.name === cic_new_lookup_tool.name) {
        toolResult = await cic_new_lookup_tool.invoke(
          toolCall.args as { phoneNumber: number },
        );
      }
      ...
      if (toolResult) {
        ...
        return `Error: Tool ${toolCall.name} was called but not found or handled.`;
      }
    } else {
      return (
        aiMsg.content?.toString() ||
        "No direct response or tool call generated."
      );
    }
  } catch (error) {
...
(async () => {
  try {
    const llmQuery = "is 0795881812 registered under CIC";
    // const llmQuery = "is 0701720503 registered under CIC";

    const llmResult = await processUserQueryWithTools(llmQuery);
